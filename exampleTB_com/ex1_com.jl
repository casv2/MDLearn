...
function plot_HMD(E_tot, E_pot, E_kin, T, P, i; k=50) # varEs,
    ...
    savefig("./exampleTB_com/HMD_$(i).pdf")
end

function run_HMD(IP, B, S, c, c_samples, at; nsteps=100, temp=100, dt=1.0, τ=0.05)
    E_tot = zeros(nsteps)
    E_pot = zeros(nsteps)
    E_kin = zeros(nsteps)
    T = zeros(nsteps)
    ...
    IPs = [SumIP(Vref, JuLIP.MLIPs.combine(B, c_samples[:,i])) for i in 1:length(c_samples[1,:])]

    while running && i < nsteps
        #at, p = MDLearn.HMD.VelocityVerlet_uncertain(IP, B, at, dt * MDLearn.MD.fs, S, c, τ=τ)
        at, p = MDLearn.HMD.VelocityVerlet_uncertain_com(IPs, at, dt * MDLearn.MD.fs, τ=τ)
...
function do_fit(al)
    ...
    m, S = MDLearn.Uncertain.posterior(Ψ, Y, 20.0, 5.0);
    c_samples = MDLearn.Uncertain.do_brr(Ψ, Y, 20.0, 5.0, 10);
...
for i in 1:20
    ...
    E_tot, E_pot, E_kin, T, P, varEs, varFs, cfgs = run_HMD(IP, B, S, c, c_samples, bulk(:Ti, cubic=true) * (2,2,2), nsteps=10000, temp=6050);
    plot_HMD(E_tot, E_pot, E_kin, T, P, i, k=1)
    write_xyz("./exampleTB_com/crash_$(i).xyz", cfgs[end])
    run(`/Users/Cas/anaconda2/bin/python /Users/Cas/.julia/dev/MDLearn/exampleTB_com/convert.py $(i)`)
end