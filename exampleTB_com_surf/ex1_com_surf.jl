...
function plot_HMD(E_tot, E_pot, E_kin, T, P, i; k=50) # varEs,
    ...
    savefig("./exampleTB_com_surf/HMD_$(i).pdf")
end

function run_HMD(IP, B, c_samples, at; nsteps=100, temp=100, dt=1.0, τ=0.01)
    E_tot = zeros(nsteps)
    E_pot = zeros(nsteps)
    E_kin = zeros(nsteps)
    T = zeros(nsteps)
...
init_at = IPFitting.Data.read_xyz(@__DIR__() * "/hcp_surf.xyz")[1].at

for i in 1:10
    init_config = deepcopy(init_at)
    al = IPFitting.Data.read_xyz(@__DIR__() * "/DB_$(i-1).xyz", energy_key="energy", force_key="force");
    IP, B, c_samples = do_fit(al)

    E_tot, E_pot, E_kin, T, P, varEs, varFs, cfgs = run_HMD(IP, B, c_samples, init_config, nsteps=10000, temp=2050);
    plot_HMD(E_tot, E_pot, E_kin, T, P, i, k=1)
    write_xyz("./exampleTB_com_surf/crash_$(i).xyz", cfgs[end])
    run(`/Users/Cas/anaconda2/bin/python /Users/Cas/.julia/dev/MDLearn/exampleTB_com_surf/convert.py $(i)`)
end
