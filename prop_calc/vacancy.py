...
from utilities import relax_config, rescale_to_relaxed_bulk, evaluate
from ase.neighborlist import NeighborList
import ase.build
import numpy as np
import spglib

def do_one_vacancy(bulk_supercell, calculator, relax_radial=0.0, relax_symm_break=0.0, nn_cutoff=0.0, tol=1.0e-2):
    ...
    vac = relax_config(vac, calculator, relax_pos=True, relax_cell=False, tol=tol, traj_file=None,
...
def do_all_vacancies(test_dir, nn_cutoff=0.0, tol=1.0e-2):
    ...
    ase.io.write(os.path.join("..","-rescaled-bulk.xyz"),  bulk_supercell, format='extxyz')
