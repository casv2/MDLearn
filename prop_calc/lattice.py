...
def calc_E_vs_V(bulk, calculator, dV=0.025, n_steps=(-10,10), tol=1e-2, method='lbfgs'): # hack tol to deal with Te C2/m
   ...
   for i in range(0, n_steps[0]-1, -1):
      ...
      try:
          #ase.io.write("E_vs_V_%03d-unrelaxed.xyz" % i,  scaled_bulk, format='extxyz')
          scaled_bulk = relax_config(scaled_bulk, calculator, relax_pos=True, relax_cell=True, tol=tol, max_steps=200, traj_file="E_vs_V_traj_%02d.extxyz" % i, constant_volume=True, method=method,
   ...
   for i in range(1,n_steps[1]+1):
      ...
      print("trying to relax i",i)
      try:
          #ase.io.write("E_vs_V_%02d-unrelaxed.xyz" % i,  scaled_bulk, format='extxyz')
          scaled_bulk = relax_config(scaled_bulk, calculator, relax_pos=True, relax_cell=True, tol=tol, max_steps=200, traj_file="E_vs_V_traj_%02d.extxyz" % i, constant_volume=True, method=method,
